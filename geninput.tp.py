...
def setval (lx,ly,tx,ty,U,J,V,Sz,N,firstmainsweep,numsweeps,mtable,para):
  ...
  while i < len(para):
    ...
    elif 'periodic =' in para[i]: para[i] = '        periodic = 1\n'
    # tprime
    elif 'extrabonds' in para[i]:
      para[i] = '    extrabonds\n'
      while 'numbonds' not in para[i]: i += 1
      para[i] = '        numbonds = '+str((lx*ly-ly)*2)+'\n'
      # delete current values
      while ' bonds' not in para[i]: i += 1
      i += 3
      while '}' not in para[i]: del para[i]
      # add new values
      def addtp (i,x,y,xp,yp):
        if yp <= 0:
          yp = ly
        elif yp > ly: yp = 1
        para.insert (i,str(x)+' '+str(y)+' '+str(xp)+' '+str(yp)+' '+str(tp)+' 0\n')
        i += 1
        return i
...
ofname = 'huben'+str(lx)+'x'+str(ly)+'.U'+str(U).rstrip('0').rstrip('.')+'.n'+str(n)+'.tp'+str(tp)+'.nt.in'
